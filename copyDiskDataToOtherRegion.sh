...
below export url is generated only when disk in detached mode. when disk is attached create incremental type snapshot and export SAS URL
...
while true; do
  ...
  if [[ "$copy_status" == "success" ]]; then
    ...
    echo "âŒ Copy failed or was aborted."
    exit 1
  fi
